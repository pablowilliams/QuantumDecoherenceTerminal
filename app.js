...
const STARRED = [
  { ticker: "HERON", name: "IBM Heron r2 — 156 qubits", sector: "Superconducting", price: 2048, mu: 0.18, sigma: 0.3 },
  { ticker: "CONDOR", name: "IBM Condor — 1121 qubits", sector: "Superconducting", price: 1024, mu: 0.12, sigma: 0.42 },
  { ticker: "WILLOW", name: "Google Willow — 105 qubits", sector: "Superconducting", price: 4096, mu: 0.22, sigma: 0.34 },
  { ticker: "FORTE", name: "IonQ Forte Enterprise — 36 ion", sector: "Trapped-ion", price: 1280, mu: 0.15, sigma: 0.22 },
  { ticker: "H2", name: "Quantinuum H2-1 — 56 ion", sector: "Trapped-ion", price: 2560, mu: 0.2, sigma: 0.18 },
  { ticker: "ANKAA", name: "Rigetti Ankaa-3 — 84 qubits", sector: "Superconducting", price: 512, mu: 0.1, sigma: 0.38 },
  { ticker: "AQUILA", name: "QuEra Aquila — 256 atoms", sector: "Neutral-atom", price: 768, mu: 0.16, sigma: 0.3 },
  { ticker: "ATOM", name: "Atom Computing — 1180 atoms", sector: "Neutral-atom", price: 896, mu: 0.24, sigma: 0.4 },
  { ticker: "OSPREY", name: "PsiQuantum Omega — photonic", sector: "Photonic", price: 384, mu: 0.3, sigma: 0.55 },
  { ticker: "ASTRA", name: "Pasqal Orion — 100 atoms", sector: "Neutral-atom", price: 640, mu: 0.18, sigma: 0.28 },
  { ticker: "MAJORANA", name: "Microsoft Majorana 1 — 8 topo", sector: "Topological", price: 128, mu: 0.45, sigma: 0.7 },
  { ticker: "TENJIN", name: "Fujitsu / RIKEN — 256 qubits", sector: "Superconducting", price: 256, mu: 0.1, sigma: 0.32 },
];
...
const SENTIMENT_POSTS = {
  "HERON": [
    {
      "sent": "pos",
      "author": "@ibm_q_research",
      "handle": "2h",
      "text": "$HERON r2 hits 2-qubit gate fidelity 99.7% on cz native — record for superconducting at this qubit count."
    },
    {
      "sent": "neu",
      "author": "@quantinuum_pap",
      "handle": "1d",
      "text": "$HERON crosstalk still dominates beyond depth 80. Dynamic decoupling helps but ceiling visible."
    }
  ],
  "CONDOR": [
    {
      ...
      "author": "@yields_q",
      "handle": "3h",
      "text": "$CONDOR 1121 qubits but median T2 only 110μs. Useful subset ~400 qubits — still impressive scale demo."
    }
  ],
  "WILLOW": [
    {
      ...
      "author": "@google_qai",
      "handle": "1h",
      "text": "$WILLOW below-threshold quantum error correction demonstrated. Logical qubit error rate scales correctly with code distance."
    },
    {
      "sent": "pos",
      "author": "@nature_phys",
      "handle": "8h",
      "text": "$WILLOW 105-qubit Willow chip RCS benchmark — 10 septillion years on best classical, 5 minutes on Willow."
    }
  ],
  "FORTE": [
    {
      ...
      "author": "@ionq_eng",
      "handle": "4h",
      "text": "$FORTE all-to-all connectivity wins on QAOA depth-32 — beats fixed-lattice SC architectures on graph problems."
    },
    {
      "sent": "neu",
      "author": "@trapped_ion",
      "handle": "1d",
      "text": "$FORTE gate times slower than SC but compilation advantage closes the gap on most algorithms."
    }
  ],
  "H2": [
    {
      ...
      "author": "@quantinuum",
      "handle": "2h",
      "text": "$H2-1 mid-circuit measurement + conditional gates working at fidelity. Real-time error correction primitives unlocked."
    }
  ],
  "ANKAA": [
    {
      ...
      "author": "@rigetti_eng",
      "handle": "5h",
      "text": "$ANKAA-3 84 qubits, 2Q fidelity 99.5% on selected pairs. Tunable couplers helping."
    }
  ],
  "AQUILA": [
    {
      ...
      "author": "@quera_lab",
      "handle": "3h",
      "text": "$AQUILA 256 atoms — analog optimization for MIS now exceeds best CPU solvers up to N=180."
    }
  ],
  "ATOM": [
    {
      ...
      "author": "@atom_comp",
      "handle": "6h",
      "text": "$ATOM 1180-atom array with mid-circuit measurement + reload. Genuine new modality for FTQC."
    }
  ],
  "OSPREY": [
    {
      ...
      "author": "@psiquantum",
      "handle": "1d",
      "text": "$OSPREY photonic — error-correction-first architecture. Single-photon detector efficiency the gating factor."
    }
  ],
  "ASTRA": [
    {
      ...
      "author": "@pasqal_ai",
      "handle": "4h",
      "text": "$ASTRA 100-atom Pasqal Orion now executing variational graph algorithms with 1000x speedup on connected MAX-CUT."
    }
  ],
  "MAJORANA": [
    {
      ...
      "author": "@ms_q_skeptic",
      "handle": "2h",
      "text": "$MAJORANA-1 8 topo qubits announced — peer review pending. Topological signature claim needs replication."
    },
    {
      "sent": "pos",
      "author": "@ms_quantum",
      "handle": "1d",
      "text": "$MAJORANA topoconductor architecture — if validated, the qubit-count-per-error-mode story changes."
    }
  ],
  "TENJIN": [
    {
      ...
      "author": "@riken_ito",
      "handle": "12h",
      "text": "$TENJIN 256-qubit Fujitsu-RIKEN chip. Solid baseline performance, real-world workload pilots ongoing."
    }
  ]
};
...
const STRATEGIES = [
  {
    id: "ma_cross",
    name: "QV MA Cross",
    desc: "20/50-period QV",
    apply: (ctx) => {
      const s20 = sma(ctx.history, 20);
      const s50 = sma(ctx.history, 50);
      if (s20 == null || s50 == null) return { signal: "MIXED", detail: "Not enough history for 50-period MA." };
      if (s20 > s50 * 1.005) return { signal: "STRONGER",  detail: `20-period (${s20.toFixed(2)}) above 50-period (${s50.toFixed(2)}) — rising trend.` };
      if (s20 < s50 * 0.995) return { signal: "WEAKER", detail: `20-period (${s20.toFixed(2)}) below 50-period (${s50.toFixed(2)}) — falling trend.` };
      return { signal: "MIXED", detail: `20-period ≈ 50-period — consolidation.` };
    },
  },
  {
    id: "rsi_mr",
    ...
    apply: (ctx) => {
      ...
      if (r < 30) return { signal: "STRONGER",  detail: `RSI ${r.toFixed(1)} — oversold, mean-revert up.` };
      if (r > 70) return { signal: "WEAKER", detail: `RSI ${r.toFixed(1)} — overbought, mean-revert down.` };
      return { signal: "MIXED", detail: `RSI ${r.toFixed(1)} — mid-range, no edge.` };
    },
  },
  {
    id: "momentum",
    name: "Fidelity Momentum",
    desc: "20-period 2Q gate",
    apply: (ctx) => {
      const h = ctx.history;
      if (h.length < 21) return { signal: "MIXED", detail: "Not enough data." };
      const r20 = (h[h.length - 1] - h[h.length - 21]) / h[h.length - 21];
      if (r20 > 0.05)  return { signal: "STRONGER",  detail: `+${(r20 * 100).toFixed(1)}% past 20d — ride the trend.` };
      if (r20 < -0.05) return { signal: "WEAKER", detail: `${(r20 * 100).toFixed(1)}% past 20d — negative momo.` };
      return { signal: "MIXED", detail: `${(r20 * 100).toFixed(1)}% past 20d — flat.` };
    },
  },
  {
    id: "mc_asymmetry",
    name: "Error Skew",
    desc: "logical-vs-physical",
    apply: (ctx) => {
      if (!ctx.mcSummary) return { signal: "MIXED", detail: "Run a simulation." };
      ...
      if (ratio > 1.3 && expectedReturn > 0.01) return { signal: "STRONGER", detail: `Up/down ratio ${ratio.toFixed(2)}x, Mean ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      if (ratio < 0.8 && expectedReturn < -0.01) return { signal: "WEAKER", detail: `Up/down ratio ${ratio.toFixed(2)}x, Mean ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      return { signal: "MIXED", detail: `Up/down ratio ${ratio.toFixed(2)}x — no clear edge.` };
    },
  },
  {
    id: "x_sentiment",
    ...
    apply: (ctx) => {
      ...
      if (s.score > 0.25)  return { signal: "STRONGER",  detail: `Net +${(s.score * 100).toFixed(0)} — note balance positive.` };
      if (s.score < -0.15) return { signal: "WEAKER", detail: `Net ${(s.score * 100).toFixed(0)} — note balance negative.` };
      return { signal: "MIXED", detail: `Net ${(s.score * 100).toFixed(0)} — mixed crowd.` };
    },
  },
];

function signalScore(signal) { return signal === "STRONGER" ? 1 : signal === "WEAKER" ? -1 : 0; }

function combineSignals(results) {
  if (!results.length) return { signal: "MIXED", detail: "Select at least one strategy." };
  const avg = results.reduce((a, r) => a + signalScore(r.signal), 0) / results.length;
  if (avg > 0.35)  return { signal: "STRONGER",    detail: "Majority positive." };
  if (avg < -0.35) return { signal: "WEAKER",   detail: "Majority negative." };
  return { signal: "MIXED", detail: "Mixed strategy signals." };
}

// ========== State ==========
const state = {
  selectedTicker: "HERON",
...
function priceFmt(x) {
  if (x == null || !isFinite(x)) return "—";
  if (x >= 1024) return "2^" + Math.round(Math.log2(x)) + " QV";
  return x.toFixed(0) + " QV";
}

function dollarFmt(x) {
  if (x == null || !isFinite(x)) return "—";
  return Math.round(x).toLocaleString() + " QV";
}

function signalBadge(signal, opts = {}) {
  const cls = signal === "STRONGER" ? "badge-buy" : signal === "WEAKER" ? "badge-sell" : "badge-unsure";
  const icon = signal === "STRONGER" ? "▲" : signal === "WEAKER" ? "▼" : "◆";
...
const kpiDefs = () => {
  ...
  return [
    { id: "kpi-value",  label: "Aggregate Q-volume", value: k.value,           fmt: (v) => dollarFmt(v), mod: "accent",                              sub: `${state.stocks.length} QPUs · equal weight` },
    { id: "kpi-pnl",    label: "Current drift",       value: k.pnl,             fmt: (v) => dollarFmt(v), mod: k.pnl >= 0 ? "positive" : "negative",  valueMod: k.pnl >= 0 ? "up" : "down", subHtml: deltaLabel(k.pnlPct) },
    { id: "kpi-er",     label: "Expected coherence change", value: k.expectedReturn,  fmt: (v) => pctFmt(v),    mod: k.expectedReturn >= 0 ? "positive" : "negative", valueMod: k.expectedReturn >= 0 ? "up" : "down", sub: `${state.horizon}-step · scenario` },
...
function renderKPIs() {
  ...
  // Right rail
  const buys = state.stocks.filter((x) => getCombinedSignalForStock(x).signal === "STRONGER").length;
  const sells = state.stocks.filter((x) => getCombinedSignalForStock(x).signal === "WEAKER").length;
  ...
  setText("#rail-er-sub", `Device set · ${state.horizon}d`);
  setText("#rail-buys-val", `${buys} / ${sells}`);
  setText("#rail-buys-sub", `${state.stocks.length} QPUs total`);
...
function renderStocksTable() {
  ...
  body.innerHTML = rows.map((s) => {
    ...
    const dotCls = s.combinedSignal.signal === "STRONGER" ? "buy" : s.combinedSignal.signal === "WEAKER" ? "sell" : "unsure";
...
function renderDetail(onComplete) {
  ...
  $("#detail-subtitle").textContent = `Architecture: ${stock.sector} · Q-volume ${priceFmt(stock.price)} (${pctFmt(stock.change)}) · Drift μ=${(stock.mu * 100).toFixed(1)}%, vol σ=${(stock.sigma * 100).toFixed(1)}%.`;
...
function renderChart(mc, stock, onComplete) {
  ...
  const s = mc.summary;
  const captionText = `${mc.nPaths.toLocaleString()} seeded scenario paths over ${mc.days} circuit cycles. Expected coherence change ${pctFmt(s.expectedReturn)}, median ${pctFmt(s.medianReturn)}, 95% CI ${pctFmt(s.ci95Low)} to ${pctFmt(s.ci95High)}, probability of gain ${(s.probUp * 100).toFixed(0)}%.`;
  ...
  body.innerHTML = [
    ...
    ["Expected coherence change", pctFmt(s.expectedReturn)],
...
function renderSentiment(stock) {
  ...
  const posts = s.posts.length ? s.posts : [{ sent: "neu", author: "@scenario", handle: "now", text: "No recent scenario notes for this QPU." }];
...
function renderSummary() {
  ...
  const buy = all.filter((x) => x === "STRONGER").length;
  const sell = all.filter((x) => x === "WEAKER").length;
  const uns = all.filter((x) => x === "MIXED").length;
  const winner = buy > sell && buy > uns ? "STRONGER" : sell > buy && sell > uns ? "WEAKER" : "MIXED";
  const agg = `${buy} ALLOCATE · ${uns} QUEUE · ${sell} DEPRECATE across ${all.length} QPUs.`;
...
function wireEvents() {
    ...
    setTimeout(() => {
      ...
      renderDetail(() => {
        ...
        const er = state.mcResult && state.mcResult.summary
          ...
          : "—";
        announce(`Simulation complete for ${state.selectedTicker}. Expected coherence change ${er}.`);
      });
    }, 30);
  });
  ...
  $("#horizon-select").addEventListener("change", (e) => {
    ...
    state.cohort = computeCohortKpis(state.stocks);
    renderAll();
    announce(`Horizon set to ${state.horizon} circuit cycles.`);
  });
...
function spawnParticles(origin, kind) {
  ...
  if (!layer) return;
  const count = kind === "STRONGER" ? 18 : 14;
  for (let i = 0; i < count; i++) {
    const p = document.createElement("div");
    p.className = "fx-particle" + (kind === "WEAKER" ? " sell" : "");
    p.style.left = origin.x + "px";
    p.style.top = origin.y + "px";
    layer.appendChild(p);
    const angle = kind === "STRONGER"
      ? (Math.random() * Math.PI * 2)
      : (Math.PI / 2 + (Math.random() - 0.5) * 0.7);
    const speed = kind === "STRONGER" ? 40 + Math.random() * 80 : 20 + Math.random() * 40;
    const dx = Math.cos(angle) * speed;
    const dy = Math.sin(angle) * speed + (kind === "WEAKER" ? 70 + Math.random() * 40 : 0);
...
function particlesForSignalFlip(row, signal) {
  if (signal !== "STRONGER" && signal !== "WEAKER") return;
  const badge = row.querySelector(".badge");
  if (!badge) return;
...
function renderExtraPanel() {
  ...
  const q = state.stocks.find((s) => s.ticker === state.selectedTicker) || state.stocks[0];
  const w = 720, h = 320, cx = 360, cy = 160, R = 130;
  // Bloch state synthesized from QPU sigma: more uncertainty -> bigger theta and smaller r (decoherence)
  const t = Date.now() / 1000;
  const phi = (t * 0.5 + q.price / 256) % (2 * Math.PI);
  const theta = (Math.PI / 3) + Math.sin(t * 0.25) * 0.4 + q.sigma * 0.6;
  const rEnv = Math.max(0.25, Math.min(1, 1 - q.sigma * 0.7));
  // Sphere projection
  const tipX = cx + R * Math.sin(theta) * Math.cos(phi);
  const tipY = cy - R * Math.cos(theta) + R * Math.sin(theta) * Math.sin(phi) * 0.35;
  const stateVecX = cx + (tipX - cx) * rEnv;
  const stateVecY = cy + (tipY - cy) * rEnv;
  // Axes
  const ax = (dx, dy, dz, label, color) => {
    const x = cx + R * dx;
    const y = cy - R * dz + R * dy * 0.35;
    return `<line x1="${cx}" y1="${cy}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}" stroke="${color}" stroke-width="1" stroke-opacity="0.8"/>
      <text x="${x.toFixed(1) + (dx >= 0 ? 8 : -8)}" y="${y.toFixed(1) + (dz >= 0 ? -6 : 14)}" fill="${color}" font-size="10" font-family="JetBrains Mono, monospace">${label}</text>`;
  };
  const ariaLabel = `Bloch sphere for ${q.ticker}. State vector at theta ${(theta * 180 / Math.PI).toFixed(0)} degrees, phi ${(phi * 180 / Math.PI).toFixed(0)} degrees. Decoherence radius ${rEnv.toFixed(2)}.`;
  container.innerHTML = `
    <svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${ariaLabel}">
      ...
      <circle cx="${cx}" cy="${cy}" r="${R * rEnv}" fill="none" stroke="#d4b5ff" stroke-width="1" stroke-dasharray="2 3"/>
      ${ax(1, 0, 0, "|+\u27e9 (x)", "#7f8693")}
      ${ax(-1, 0, 0, "|-\u27e9", "#7f8693")}
      ${ax(0, 1, 0, "|+i\u27e9 (y)", "#7f8693")}
      ${ax(0, -1, 0, "|-i\u27e9", "#7f8693")}
      ${ax(0, 0, 1, "|0\u27e9 (z)", "#d6dce4")}
      ${ax(0, 0, -1, "|1\u27e9", "#d6dce4")}
      <line x1="${cx}" y1="${cy}" x2="${stateVecX.toFixed(1)}" y2="${stateVecY.toFixed(1)}" stroke="#b48cff" stroke-width="2.5"/>
      <circle cx="${stateVecX.toFixed(1)}" cy="${stateVecY.toFixed(1)}" r="5" fill="#b48cff"/>
      <text x="16" y="22" fill="#7f8693" font-size="10" font-family="JetBrains Mono, monospace">${q.ticker} · |\u03c8\u27e9 = cos(${(theta/2).toFixed(2)})|0\u27e9 + e^{i${phi.toFixed(2)}} sin(${(theta/2).toFixed(2)})|1\u27e9</text>
      <text x="16" y="${h - 14}" fill="#7f8693" font-size="10" font-family="JetBrains Mono, monospace">decoherence radius |r| = ${rEnv.toFixed(3)} · T2 contraction visible</text>
    </svg>
  `;
  const tk = document.getElementById("extra-ticker"); if (tk) tk.textContent = q.ticker;
  const sub = document.getElementById("extra-subtitle");
  if (sub) sub.textContent = `${q.name} · sigma=${(q.sigma * 100).toFixed(0)}% gate-error proxy · theta=${(theta * 180 / Math.PI).toFixed(0)}°, phi=${(phi * 180 / Math.PI).toFixed(0)}° · |r|=${rEnv.toFixed(3)}.`;
  const body = document.getElementById("extra-data-body");
  if (body) body.innerHTML = [
    ["X (\u27e8\u03c3_x\u27e9)", (Math.sin(theta) * Math.cos(phi) * rEnv).toFixed(3)],
    ["Y (\u27e8\u03c3_y\u27e9)", (Math.sin(theta) * Math.sin(phi) * rEnv).toFixed(3)],
    ["Z (\u27e8\u03c3_z\u27e9)", (Math.cos(theta) * rEnv).toFixed(3)],
  ].map(([k, v]) => `<tr><td>${k}</td><td>${v}</td></tr>`).join("");
  // Reduced-motion: render once, no rAF tick.
  if (!window.matchMedia || !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    if (state._blochRaf) cancelAnimationFrame(state._blochRaf);
    state._blochRaf = requestAnimationFrame(() => {
      if (state.selectedTicker === q.ticker && document.getElementById("extra-content")) renderExtraPanel();
    });
  }
}
...
function renderExtra2Panel() {
  ...
  const rowH = inH / rows.length;
  // T1, T2 → gate budgets (microseconds / 100ns per gate ~= 10x scale)
  const ledger = rows.map(q => {
    const T1 = Math.round(40 + (1 - q.sigma) * 200);
    const T2 = Math.round(T1 * (0.4 + (1 - q.sigma) * 0.4));
    const gateT1 = Math.round(T1 * 0.6 / 0.1);
    const gateT2 = Math.round(T2 * 0.6 / 0.1);
    return { ticker: q.ticker, T1, T2, gateT1, gateT2 };
  });
  const maxG = Math.max(...ledger.flatMap(l => [l.gateT1, l.gateT2]));
  let bars = "";
  ledger.forEach((l, i) => {
    const y = padT + i * rowH;
    const w1 = (l.gateT1 / maxG) * (inW * 0.6);
    const w2 = (l.gateT2 / maxG) * (inW * 0.6);
    bars += `<text x="${padL - 6}" y="${y + rowH / 2 + 3}" text-anchor="end" fill="#d6dce4" font-size="10" font-family="JetBrains Mono, monospace">${l.ticker}</text>
      <rect x="${padL}" y="${y + 4}" width="${w1.toFixed(1)}" height="${(rowH / 2 - 4).toFixed(1)}" fill="#b48cff" fill-opacity="0.7"/>
      <text x="${padL + w1 + 8}" y="${y + rowH / 2 - 2}" fill="#b48cff" font-size="10" font-family="JetBrains Mono, monospace">T1: ${l.gateT1} gates · ${l.T1}μs</text>
      <rect x="${padL}" y="${y + rowH / 2 + 2}" width="${w2.toFixed(1)}" height="${(rowH / 2 - 4).toFixed(1)}" fill="#d4b5ff" fill-opacity="0.6"/>
      <text x="${padL + w2 + 8}" y="${y + rowH - 4}" fill="#d4b5ff" font-size="10" font-family="JetBrains Mono, monospace">T2: ${l.gateT2} gates · ${l.T2}μs</text>`;
  });
  container.innerHTML = `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Circuit depth ledger by QPU. T1-limited and T2-limited gate counts shown per processor.">
    <text x="${padL}" y="14" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">CIRCUIT DEPTH BUDGET · upper = T1 limit · lower = T2 limit · 100ns gate</text>
    ${bars}
  </svg>`;
